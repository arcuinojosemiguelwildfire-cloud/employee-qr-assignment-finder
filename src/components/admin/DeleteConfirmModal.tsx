...
export const DeleteConfirmModal: React.FC<DeleteConfirmModalProps> = ({
  ...
  const displayName = employee.name || employee.employee_name;
  const tablesText = employee.tables && employee.tables.length > 0 
    ? employee.tables.join(', ') 
    : 'No tables assigned';
        ...
        >
          ...
          <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 mb-6 text-left">
            ...
            <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-slate-600 mt-2">
              <span className="font-semibold">MEM Group:</span>
              <span className="bg-indigo-50 text-indigo-700 px-1.5 py-0.5 rounded font-bold">{employee.mem_group}</span>
              <span>•</span>
              <span className="font-semibold">Priority:</span>
              <span className="bg-slate-200/70 text-slate-800 px-1.5 py-0.5 rounded font-bold">{employee.mem_priority_group}</span>
            </div>
            <div className="text-[11px] text-slate-500 mt-1.5">
              <span className="font-semibold text-slate-600">Table(s):</span> {tablesText}
            </div>
          </div>
